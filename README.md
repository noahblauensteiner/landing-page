...
| `api/chat.ts` | Streaming chat endpoint (Vercel Edge Function). Calls the Claude API (`claude-opus-4-8`) server-side — the API key never reaches the browser. |
...
| `vite.config.ts` | Mounts the same `api/chat.ts` handler on the dev server, so `npm run dev` gives you the full experience locally. |
...
Designed for [Vercel](https://vercel.com): import the repo, set the `ANTHROPIC_API_KEY`
environment variable, deploy. The `api/` directory is picked up automatically as an Edge
Function. Any platform that supports Vite static builds + edge/serverless functions works
the same way.
